...
use crate::{
	...
	start_rpc_servers, build_network_future, TaskManager, SpawnTaskHandle,
...
use log::info;
use ac_network::config::Role;
use ac_network::NetworkService;
...
// 	SpawnNamed,
// };
// use ap_keystore::{CryptoStore, SyncCryptoStore, SyncCryptoStorePtr};
...
pub trait RpcExtensionBuilder {
	...
	type Output: ac_rpc::RpcExtension<ac_rpc::Metadata>;
	...
	fn build(
		&self,
		deny: ac_rpc::DenyUnsafe,
		subscription_executor: ac_rpc::SubscriptionTaskExecutor,
	) -> Self::Output;
}

impl<F, R> RpcExtensionBuilder for F where
	F: Fn(ac_rpc::DenyUnsafe, ac_rpc::SubscriptionTaskExecutor) -> R,
	R: ac_rpc::RpcExtension<ac_rpc::Metadata>,
{
	type Output = R;
...
impl<R> RpcExtensionBuilder for NoopRpcExtensionBuilder<R> where
	R: Clone + ac_rpc::RpcExtension<ac_rpc::Metadata>,
{
	type Output = R;

	fn build(
		&self,
		_deny: ac_rpc::DenyUnsafe,
		_subscription_executor: ac_rpc::SubscriptionTaskExecutor,
	) -> Self::Output {
		self.0.clone()
	}
}
...
pub struct SpawnTasksParams<'a, TRpc> {
	...
	pub network_status_sinks: NetworkStatusSinks,
	/// A Sender for RPC requests.
	pub system_rpc_tx: TracingUnboundedSender<ac_rpc::system::Request>,
...
pub fn spawn_tasks<TRpc>(
	params: SpawnTasksParams<TRpc>,
) -> Result<RpcHandlers, Error>
	where
		TRpc: ac_rpc::RpcExtension<ac_rpc::Metadata>
{
	...
	// RPC
	let gen_handler = |
		deny_unsafe: ac_rpc::DenyUnsafe,
		rpc_middleware: ac_rpc_server::RpcMiddleware
	| gen_handler(
		deny_unsafe, rpc_middleware, &config, task_manager.spawn_handle(),
		&*rpc_extensions_builder,
		system_rpc_tx.clone()
	);
	let rpc_metrics = ac_rpc_server::RpcMetrics::new(config.prometheus_registry())?;
	...
	let rpc_handlers = RpcHandlers(Arc::new(gen_handler(
		...
		ac_rpc_server::RpcMiddleware::new(rpc_metrics, "inbrowser")
	).into()));
...
fn gen_handler<TRpc>(
	...
	system_rpc_tx: TracingUnboundedSender<ac_rpc::system::Request>
		...
		TRpc: ac_rpc::RpcExtension<ac_rpc::Metadata>,
{
	...
	let system_info = ac_rpc::system::SystemInfo {
	...
	let task_executor = ac_rpc::SubscriptionTaskExecutor::new(spawn_handle);
	...
	// 	// Light clients
	// 	let chain = ac_rpc::chain::new_light(
	// 		client.clone(),
	// 		subscriptions.clone(),
	// 		remote_blockchain.clone(),
	// 		on_demand.clone(),
	// 	);
	// 	let (state, child_state) = ac_rpc::state::new_light(
	// 		client.clone(),
	// 		subscriptions.clone(),
	// 		remote_blockchain.clone(),
	// 		on_demand,
	...
	// } else {
	// 	// Full nodes
	// 	let chain = ac_rpc::chain::new_full(client.clone(), subscriptions.clone());
	// 	let (state, child_state) = ac_rpc::state::new_full(
	// 		client.clone(),
	// 		subscriptions.clone(),
	// 		deny_unsafe,
	// 	);
	...
	// let author = ac_rpc::author::Author::new(
	// 	client,
	// 	transaction_pool,
	// 	subscriptions,
	// 	keystore,
	...
	// 	let offchain = ac_rpc::offchain::Offchain::new(storage, deny_unsafe);
	// 	offchain::OffchainApi::to_delegate(offchain)
	// });

	ac_rpc_server::rpc_handler(
		(
...
pub struct BuildNetworkParams<'a> {
	/// The service configuration.
...
pub fn build_network(
	...
	(
		Arc<NetworkService>,
		NetworkStatusSinks,
		TracingUnboundedSender<ac_rpc::system::Request>,
		NetworkStarter,
	),
	Error
> {
	...
	let network_params = ac_network::config::Params {
	...
	let network_mut = ac_network::NetworkWorker::new(network_params)?;
