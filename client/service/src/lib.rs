...
//! Anonima service. Starts a thread that spins up the network.
...
use ac_network::{NetworkStatus, network_state::NetworkState, PeerId};
...
pub use self::builder::{
	spawn_tasks, build_network, BuildNetworkParams, NetworkStarter, SpawnTasksParams,
	RpcExtensionBuilder, NoopRpcExtensionBuilder
};
pub use config::{
	BasePath, Configuration, Role, RpcMethods, TaskExecutor, TaskType,
};
...
pub use ac_rpc::Metadata as RpcMetadata;
pub use sc_executor::NativeExecutionDispatch;
#[doc(hidden)]
pub use std::{ops::Deref, result::Result, sync::Arc};
#[doc(hidden)]
// pub use ac_network::config::{
...
#[derive(Clone)]
pub struct RpcHandlers(Arc<jsonrpc_core::MetaIoHandler<ac_rpc::Metadata, ac_rpc_server::RpcMiddleware>>);
...
impl RpcHandlers {
	...
	pub fn io_handler(&self)
		-> Arc<jsonrpc_core::MetaIoHandler<ac_rpc::Metadata, ac_rpc_server::RpcMiddleware>> {
		self.0.clone()
	}
}
...
#[derive(Clone)]
pub struct NetworkStatusSinks {
	status: Arc<status_sinks::StatusSinks<NetworkStatus>>,
	state: Arc<status_sinks::StatusSinks<NetworkState>>,
}

impl NetworkStatusSinks {
	fn new() -> Self {
		Self {
	...
	pub fn status_stream(&self, interval: Duration)
		-> TracingUnboundedReceiver<NetworkStatus>
	{
...
async fn build_network_future(
	role: Role,
	mut network: ac_network::NetworkWorker,
	// client: Arc<C>,
	status_sinks: NetworkStatusSinks,
	mut rpc_rx: TracingUnboundedReceiver<ac_rpc::system::Request>,
	...
	loop {
		futures::select!{
			...
			request = rpc_rx.select_next_some() => {
				match request {
					ac_rpc::system::Request::Health(sender) => {
						let _ = sender.send(ac_rpc::system::Health {
							peers: network.peers_debug_info().len(),
							// is_syncing: network.service().is_major_syncing(),
							should_have_peers,
						});
					},
					ac_rpc::system::Request::LocalPeerId(sender) => {
						let _ = sender.send(network.local_peer_id().to_base58());
					},
					ac_rpc::system::Request::LocalListenAddresses(sender) => {
						let peer_id = network.local_peer_id().clone().into();
						let p2p_proto_suffix = ac_network::multiaddr::Protocol::P2p(peer_id);
						let addresses = network.listen_addresses()
							.map(|addr| addr.clone().with(p2p_proto_suffix.clone()).to_string())
							.collect();
						let _ = sender.send(addresses);
					},
					ac_rpc::system::Request::Peers(sender) => {
						let _ = sender.send(network.peers_debug_info().into_iter().map(|(peer_id, p)|
							ac_rpc::system::PeerInfo {
					...
					ac_rpc::system::Request::NetworkState(sender) => {
						if let Some(network_state) = serde_json::to_value(&network.network_state()).ok() {
							let _ = sender.send(network_state);
						}
					}
					ac_rpc::system::Request::NetworkAddReservedPeer(peer_addr, sender) => {
						let x = network.add_reserved_peer(peer_addr)
							.map_err(ac_rpc::system::error::Error::MalformattedPeerArg);
						let _ = sender.send(x);
					}
					ac_rpc::system::Request::NetworkRemoveReservedPeer(peer_id, sender) => {
						let _ = match peer_id.parse::<PeerId>() {
							Ok(peer_id) => {
								network.remove_reserved_peer(peer_id);
								sender.send(Ok(()))
							}
							Err(e) => sender.send(Err(ac_rpc::system::error::Error::MalformattedPeerArg(
								e.to_string(),
							))),
						};
					}
					ac_rpc::system::Request::NodeRoles(sender) => {
						use ac_rpc::system::NodeRole;
					...
					// ac_rpc::system::Request::SyncState(sender) => {
					// 	// use ac_rpc::system::SyncState;
...
mod waiting {
	pub struct HttpServer(pub Option<ac_rpc_server::HttpServer>);
	...
	pub struct IpcServer(pub Option<ac_rpc_server::IpcServer>);
	...
	pub struct WsServer(pub Option<ac_rpc_server::WsServer>);
...
fn start_rpc_servers<
	H: FnMut(ac_rpc::DenyUnsafe, ac_rpc_server::RpcMiddleware)
	-> ac_rpc_server::RpcHandler<ac_rpc::Metadata>
>(
	config: &Configuration,
	mut gen_handler: H,
	rpc_metrics: ac_rpc_server::RpcMetrics,
	...
	fn deny_unsafe(addr: &SocketAddr, methods: &RpcMethods) -> ac_rpc::DenyUnsafe {
		let is_exposed_addr = !addr.ip().is_loopback();
		match (is_exposed_addr, methods) {
			| (_, RpcMethods::Unsafe)
			| (false, RpcMethods::Auto) => ac_rpc::DenyUnsafe::No,
			_ => ac_rpc::DenyUnsafe::Yes
		}
	}

	Ok(Box::new((
		config.rpc_ipc.as_ref().map(|path| ac_rpc_server::start_ipc(
			&*path, gen_handler(
				ac_rpc::DenyUnsafe::No,
				ac_rpc_server::RpcMiddleware::new(rpc_metrics.clone(), "ipc")
			)
		)),
		maybe_start_server(
			config.rpc_http,
			|address| ac_rpc_server::start_http(
				address,
				config.rpc_cors.as_ref(),
				gen_handler(
					deny_unsafe(&address, &config.rpc_methods),
					ac_rpc_server::RpcMiddleware::new(rpc_metrics.clone(), "http")
				),
			),
		)?.map(|s| waiting::HttpServer(Some(s))),
		maybe_start_server(
			config.rpc_ws,
			|address| ac_rpc_server::start_ws(
				address,
				config.rpc_ws_max_connections,
				config.rpc_cors.as_ref(),
				gen_handler(
					deny_unsafe(&address, &config.rpc_methods),
					ac_rpc_server::RpcMiddleware::new(rpc_metrics.clone(), "ws")
				),
			),
...
// impl<B, H, C, Pool, E> ac_network::config::TransactionPool<H, B> for
// 	TransactionPoolAdapter<C, Pool>
// where
// 	C: ac_network::config::Client<B> + Send + Sync,
